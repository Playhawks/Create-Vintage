...
import dev.bluephs.vintage.content.kinetics.curving_press.CurvingBehaviour.Mode;
...
public class BeltCurvingCallbacks {
	...
	static ProcessingResult onItemReceived(TransportedItemStack transported,
		TransportedItemStackHandlerBehaviour handler, CurvingBehaviour behaviour) {
	...
	static ProcessingResult whenItemHeld(TransportedItemStack transported, TransportedItemStackHandlerBehaviour handler,
		CurvingBehaviour behaviour) {

		if (behaviour.specifics.getKineticSpeed() == 0)
			return PASS;
		if (!behaviour.running)
			return PASS;
		if (behaviour.runningTicks != CurvingBehaviour.CYCLE / 2)
			return HOLD;
