...
public class GrinderSandpaperPolishingCategory extends CreateRecipeCategory<SandPaperPolishingRecipe> {
	...
	public void draw(SandPaperPolishingRecipe recipe, IRecipeSlotsView iRecipeSlotsView, GuiGraphics graphics, double mouseX, double mouseY) {
		...
		switch (speedLimits) {
			case 1 ->
					graphics.drawCenteredString(Minecraft.getInstance().font, Component.translatable(Vintage.MOD_ID + ".jei.text.required_speed").append(" ").append(Component.translatable(Vintage.MOD_ID + ".jei.text.low")), 88, 75, 0x00FF00);
			case 2 ->
					graphics.drawCenteredString(Minecraft.getInstance().font, Component.translatable(Vintage.MOD_ID + ".jei.text.required_speed").append(" ").append(Component.translatable(Vintage.MOD_ID + ".jei.text.medium")), 88, 75, 0xFFFF00);
			case 3 ->
					graphics.drawCenteredString(Minecraft.getInstance().font, Component.translatable(Vintage.MOD_ID + ".jei.text.required_speed").append(" ").append(Component.translatable(Vintage.MOD_ID + ".jei.text.high")), 88, 75, 0xFF0000);
			default ->
					graphics.drawCenteredString(Minecraft.getInstance().font, Component.translatable(Vintage.MOD_ID + ".jei.text.required_speed").append(" ").append(Component.translatable(Vintage.MOD_ID + ".jei.text.any")), 88, 75, 0xFFFFFF);
		}
		...
		if (VintageConfig.server().recipes.destroyOnWrongGrinderSpeed.get()) {
			...
			if (mouseX >= 2 && mouseX <= 15 && mouseY >= 62 && mouseY <= 85)
				graphics.renderTooltip(Minecraft.getInstance().font,
						Component.translatable(Vintage.MOD_ID + ".jei.text.fragile"), (int)mouseX,  (int)mouseY);
		}
	}
